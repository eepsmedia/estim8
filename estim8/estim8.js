...
const estim8 = {
    ...
    endGame: function () {
        ...
        const tMessage = "Game over! Your score is " + this.state.currentScore.toFixed(2);
        alert(tMessage);
     ...
     * updates the score, records the results
     */
    endTurn: function () {

        if (estim8.state.playing) {     // nb: unnecessary if the button is hidden if not playing!

            const valueFieldString = document.getElementById("valueField").value;

            if (!isNaN(valueFieldString) && valueFieldString.length > 0 ) {
                estim8.state.lastInputNumber =  Number(valueFieldString);
                const tDelta = this.state.currentTruth - this.state.lastInputNumber;
                ...
                console.log("Turn " + this.state.turnNumber + " score " + this.state.currentScore.toFixed(2));
...
estim8.codapSelects = function (iMessage) {      //  |N| part of session 2 solution
    let tMessageValue = iMessage.values;
...
estim8.stripView = {
     ...
     * Not needed in the basic tutorial unless you want to "reverse" the sense of the estimation.
     * @param iEvent
     */
    click: function (iEvent) {
        const tWhere = findLocalPoint(iEvent);      //      defined below
...
function findLocalPoint(iEvent) {
    let uupos = iEvent.currentTarget.createSVGPoint();
    uupos.x = iEvent.clientX;
    uupos.y = iEvent.clientY;   //  we will not use this!

    const ctmInverse = iEvent.target.getScreenCTM().inverse();
...
async function startCodapConnection() {
    ...
    try {
        ...
        estim8.state = await codapInterface.getInteractiveState();  // |S| initialize state variable!
    } catch (msg) {
